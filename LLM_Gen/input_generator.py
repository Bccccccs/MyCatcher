...
from src.progress import print_progress, should_report_progress
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Starting test file index, e.g. 90 writes test_090.in first",
    )
    ...
    if args.start_index < 0:
        raise ValueError("Invalid --start-index: require start-index >= 0")
    ...
    client = get_client()
    problem_label = spec_path.parent.name or out_dir.name or "unknown"

    print(f"[INFO] using template: {tmpl_path}")
    want_generator = is_generator_template(tmpl_path)
    print_progress(0, args.num, f"pid={problem_label} generated=0/{args.num} out={out_dir}")
    ...
    while written < args.num:
        prompt = build_prompt(template, pro_des=pro_des, code=code, num=1)
        ...
        if want_generator:
            ...
            if not gen_code.strip():
                ...
                total_failures += 1
                print(f"[WARN] empty generator code, failure={total_failures}")
                if consecutive_failures > args.max_retry:
                    break
                time.sleep(args.sleep)
                continue

            try:
                cases = run_generator_code(gen_code, num=1, timeout_sec=12)
            except Exception as e:
                consecutive_failures += 1
                total_failures += 1
                print(f"[WARN] generator execution failed ({e}), failure={total_failures}")
                if consecutive_failures > args.max_retry:
                    break
                time.sleep(args.sleep)
                continue
            ...
            if not cases:
                consecutive_failures += 1
                total_failures += 1
                print(f"[WARN] no inputs parsed, failure={total_failures}")
                if consecutive_failures > args.max_retry:
                    break
                time.sleep(args.sleep)
                continue
        ...
        case = cases[0]
        p = out_dir / f"test_{next_index:03d}.in"
        p.write_text(case, encoding="utf-8")
        written += 1
        next_index += 1
        consecutive_failures = 0
        if should_report_progress(written, args.num):
            print_progress(
                written,
                args.num,
                f"pid={problem_label} generated={written}/{args.num} latest={p.name}",
            )
        if args.sleep > 0:
            time.sleep(args.sleep)
    ...
    if written < args.num:
        print(f"[WARN] wrote only {written}/{args.num} inputs before retries were exhausted")
